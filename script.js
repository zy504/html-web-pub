...
function selectSize(button, productId) {
    ...
    const sizeButtons = productCard.querySelectorAll('.Esize-options button');
    ...
    selectedSizes[productId] = button.textContent;
    
    // Hide any size warning
    const warning = productCard.querySelector('.size-warning');
    if (warning) {
        warning.style.display = 'none';
    }
    
    // Enable the add to cart button
    const addToCartBtn = productCard.querySelector('.Eadd-to-cart');
    addToCartBtn.disabled = false;
}

// Modified addToCart function to check for size selection
function addToCart(productName, price, imageUrl, button) {
    const productCard = button.closest('.Eproduct-card');
    const productId = productCard.querySelector('a').getAttribute('href').substring(1); // Get product ID from href
    
    // Check if size is selected (only for products that have sizes)
    const hasSizes = productCard.querySelector('.Esize-options');
    if (hasSizes && !selectedSizes[productId]) {
        ...
        return; // Don't add to cart without size
    }
    
    // Add size info to product name if selected
    ...
    console.log(`Added to cart: ${fullProductName} - $${price}`);
